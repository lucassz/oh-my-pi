...
- If payload text intentionally starts with a space, write two leading spaces after `{{hsep}}` (e.g. `{{hsep}}  indented`); the first space is payload, not padding.
- Payload text is otherwise verbatim — NEVER escape unicode.
